...
# ==========================
# RUTAS BASE Y HELPERS
# ==========================
APP_DIR = Path(__file__).resolve().parent
ROOT_DIR = APP_DIR
NB_DIR   = APP_DIR / "notebooks"

def find_file(*names):
    """
    Busca un archivo por nombre en la raíz del repo y en notebooks/.
    Retorna Path o None.
    """
    for nm in names:
        for base in (ROOT_DIR, NB_DIR):
            p = base / nm
            if p.exists():
                return p
    return None
...
def find_folder(*names):
    """
    Busca una carpeta por nombre en la raíz del repo y en notebooks/.
    Retorna Path o None.
    """
    for nm in names:
        for base in (ROOT_DIR, NB_DIR):
...
POSTER_FOLDER = "Posterss"  # nombre de carpeta con pósters
...
def _poster_index():
    """Indexa Posterss en raíz o en notebooks/."""
    base = find_folder(POSTER_FOLDER)
    files = []
    if base:
    ...
    return (base or (APP_DIR / POSTER_FOLDER)), files, names_ci
...
with st.sidebar.expander("🔧 Diagnóstico (click para abrir)", expanded=True):
    ...
    try:
        ...
        for fname in [
            ...
            if p:
                try:
                    found.append(str(p.relative_to(APP_DIR)))
                except Exception:
                    found.append(str(p))
        if found:
            st.success(f"Encontrado(s): {found}")
        else:
            st.error("No se encontró ningún archivo *_clustered.csv en raíz ni en notebooks/")
            st.info("Coloca junto a app.py o en notebooks/ alguno de: "
                    "`best_method_clustered.csv`, "
                    "`combined_features_clustered.csv`, "
                    "`pca_features_clustered.csv`, "
                    "`umap_features_clustered.csv`.")
...
def load_clustered_data():
    """Carga el dataset clusterizado disponible y devuelve df, feature_cols, método, nombre_archivo."""
    candidates = [
    ...
    if 'cluster' not in df.columns:
        return None, None, None, str(chosen[0].name)

    return df, feature_cols, chosen[1], str(chosen[0].name)
...
# ==========================
# PLOTS (ejes con nombres legibles)
# ==========================
...
def main():
    ...
    if df is None:
        ...
        Asegúrate de tener alguno de estos archivos en **raíz** o en **notebooks/** y que incluya la columna `cluster`:
        - `best_method_clustered.csv`
        - `combined_features_clustered.csv`
        - `pca_features_clustered.csv`
        - `umap_features_clustered.csv`
    ...
    with st.sidebar.expander("🖼️ Diagnóstico de pósters", expanded=False):
        try:
            ...
            st.write(f"Carpeta de pósters detectada: {base}")
            ...
            if view_option == "Lista":
                ...
                if len(numeric_cols_f) >= 2:
                    ...
                    with cy:
                        ...
                        default_y_idx = 1 if numeric_cols_f and numeric_cols_f[0] == x_f and len(numeric_cols_f) > 1 else 0
